...
CREATE SEQUENCE mente.form_object_id_seq;
CREATE TABLE mente.form_object_info (
  form_object_id INTEGER NOT NULL DEFAULT nextval('mente.form_object_id_seq'::regclass),
  object_type varchar(10) NOT NULL DEFAULT '',
  object_key varchar(30) NOT NULL DEFAULT '',
  class_name varchar(50) NOT NULL DEFAULT '',
  idx INTEGER NOT NULL DEFAULT 0,
  page_id INTEGER NOT NULL DEFAULT 0,
  CONSTRAINT pk_form_object_id PRIMARY KEY (form_object_id),
  CONSTRAINT fk_form_object_info_page_id FOREIGN KEY (page_id) REFERENCES mente.page_info(page_id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION
);
COMMENT ON TABLE mente.form_object_info IS 'フォーム情報';
COMMENT ON COLUMN mente.form_object_info.form_object_id IS 'フォームID';
COMMENT ON COLUMN mente.form_object_info.page_id IS 'ページID';
...
CREATE TABLE mente.schema_info (
  ...
  schema_key varchar(30) NOT NULL DEFAULT '',
  idx INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE mente.properties_info (
  ...
  CONSTRAINT pk_properties_name PRIMARY KEY (properties_name),
...
CREATE TABLE mente.ui_info (
  ...
  CONSTRAINT pk_ui_info_properties_name PRIMARY KEY (properties_name),
...
CREATE TABLE mente.default_data_info (
  ...
  CONSTRAINT pk_default_data_info_properties_name PRIMARY KEY (properties_name),
...
CREATE TABLE mente.base64_info (
  ...
  updated_id INTEGER DEFAULT NULL,
  updated_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT pk_base64_properties_id PRIMARY KEY (properties_id),
  CONSTRAINT fk_base64_info_properties_id FOREIGN KEY (properties_id) REFERENCES mente.properties_info(properties_id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION
);
COMMENT ON TABLE mente.base64_info IS 'ファイルをBase64に変換情報';
COMMENT ON COLUMN mente.base64_info.properties_id IS 'FieldID';
...
CREATE TABLE mente.edit_object_info (
  ...
  value jsonb,
  CONSTRAINT pk_edit_object_info_properties_name PRIMARY KEY (properties_name),
...
CREATE TABLE mente.label_info (
  ...
  object_label jsonb,
  CONSTRAINT pk_label_info_properties_name PRIMARY KEY (properties_name)
);
