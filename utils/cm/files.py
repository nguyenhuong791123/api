...
def put_ftp_sftp_files(mode, sftp, transport, lpath, rpath, files, flag):
    result = []
    if sftp is None  or rpath is None:
        return result
    if transport is None and mode:
        return result
    if lpath is None:
        lpath = '.'
                ...
                if os.path.isfile(local):
                    if mode:
                        ...
                        with open(local, "rb") as f:
                            print(cmd)
                            sftp.storlines(cmd, f)
                            # f.close()
                        # f = open(local, 'rb')
                        # filename = os.path.basename(file)
                        # sftp.storbinary(cmd, f)
                        # f.close()
                else:
    ...
    if sftp is not None:
        if mode:
            ...
            sftp.quit()
    if transport is not None and mode == False:
        transport.close()
